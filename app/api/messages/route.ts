...
export async function GET(req: NextRequest){
  ...
  const { data, error } = await admin.from('messages').select('*').eq('user_id', userId).eq('thread_id', threadId).order('created_at', { ascending: true })
...
export async function POST(req: NextRequest){
  ...
  const admin = supabaseAdmin()
  if(!admin) return new Response('Missing SUPABASE_SERVICE_ROLE', { status: 500 })
  const { data, error } = await admin.from('messages').insert({ user_id: userId, thread_id, role, content }).select().single()
  if(error) return new Response(error.message, { status: 500 })
