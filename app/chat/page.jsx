...
export default function ChatPage() {
  ...
  async function generateImage() {
    ...
    } catch (e) { setToast('خطأ في التوليد'); } finally { setBusy(false); }
  }
  ...
  async function generateAd() {
    ...
    try {
      ...
      } else { setToast('فشل توليد الإعلان'); }
    } catch (e) { setToast('خطأ في توليد الإعلان'); } finally { setBusy(false); }
  }
      ...
      <main className="flex-1 flex flex-col">
        <header className="p-2 border-b border-[#333] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="text-xs text-gray-400">v1.2.1</div>
            <div className="relative">
        ...
        <footer className="p-3 border-t border-[#333]">
          <div className="max-w-4xl mx-auto w-full">
            <div className="composer relative p-2">
              ...
              {/* Drop-up menu on plus */}
              <div className="absolute right-2 top-2">
                <IconBtn title="قائمة الإضافة" onClick={()=>setMenuOpen(v=>!v)}><SvgPlus /></IconBtn>
                {menuOpen && (
          ...
          {/* Modals */}
          {imgModal && (
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-30" onClick={()=>!busy && setImgModal(false)}>
              <div onClick={(e)=>e.stopPropagation()} className="bg-bgsoft border border-[#333] rounded p-4 w-[520px] max-w-[92vw] space-y-3">
                ...
                <div className="flex justify-end gap-2">
                  <button onClick={()=>setImgModal(false)} className="px-3 py-1 rounded hover:bg-bgdark" disabled={busy}>إغلاق</button>
                  <button onClick={generateImage} className="px-3 py-1 rounded bg-neon text-black" disabled={busy}>توليد</button>
                </div>
              </div>
            </div>
          )}
          ...
          {adModal && (
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-30" onClick={()=>!busy && setAdModal(false)}>
              <div onClick={(e)=>e.stopPropagation()} className="bg-bgsoft border border-[#333] rounded p-4 w-[560px] max-w-[92vw] space-y-3">
                ...
                <div className="flex justify-end gap-2">
                  <button onClick={()=>setAdModal(false)} className="px-3 py-1 rounded hover:bg-bgdark" disabled={busy}>إغلاق</button>
                  <button onClick={generateAd} className="px-3 py-1 rounded bg-neon text-black" disabled={busy}>توليد</button>
                </div>
              </div>
            </div>
          )}
